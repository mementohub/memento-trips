...
@php
    ...
    if ($booking->service_id) {
        ...
        foreach ($ageBreakdown as $key => $row) {
            $pax[$key] = (int)($row['qty'] ?? 0);
        }
        if (array_sum($pax) === 0) {
        ...
        foreach ($serviceExtras as $ex) {
            $isMandatory = (bool)($ex->is_mandatory ?? false);
            $isTax       = (bool)($ex->is_tax ?? false);
            $priceType   = (string)($ex->price_type ?? 'flat');
            $applyAll    = (bool)($ex->apply_to_all_persons ?? false);
            $price       = (float)($ex->price ?? 0);
            $lineAmt     = 0;

            if ($isTax) {
                $pct = (float)($ex->tax_percentage ?? 0);
                $lineAmt = round(($pct / 100) * $ticketSum, 2);
            } elseif (in_array($priceType, ['flat','per_booking'])) {
                $lineAmt = $price;
            } elseif ($priceType === 'per_person') {
                $adultP  = (float)($ex->adult_price ?? $price);
                $childP  = (float)($ex->child_price ?? 0);
                $infantP = (float)($ex->infant_price ?? 0);
                $babyP   = (float)($ex->baby_price ?? 0);
                if ($adultP > 0 || $childP > 0 || $infantP > 0 || $babyP > 0) {
                    $lineAmt = ($pax['adult'] * $adultP) + ($pax['child'] * $childP) + ($pax['infant'] * $infantP) + ($pax['baby'] * $babyP);
                } else {
                    $lineAmt = $price * $totalPax;
                }
            }

            $lineAmt = round($lineAmt, 2);
            if ($lineAmt > 0 || $isMandatory) {
    ...
    // Pickup point charge reconstruction
...
{{-- Header --}}
<div class="bd-header">
    <a href="{{ route('admin.tourbooking.bookings.index') }}" class="bd-back"><i class="fa fa-arrow-left"></i> {{ __('translate.Back') }}</a>
...
<div class="bd-actions">
    <a href="{{ route('admin.tourbooking.bookings.invoice', $booking->id) }}" target="_blank" class="bd-btn"><i class="fa fa-file-invoice"></i> {{ __('translate.View Invoice') }}</a>
    <a href="{{ route('admin.tourbooking.bookings.download-invoice', $booking->id) }}" class="bd-btn"><i class="fa fa-download"></i> {{ __('translate.Download Invoice') }}</a>
    <a href="#" class="bd-btn" data-bs-toggle="modal" data-bs-target="#paymentStatusModal"><i class="fa fa-credit-card"></i> {{ __('translate.Change Payment Status') }}</a>
    @if (in_array(strtolower((string)$booking->booking_status), ['pending','success']))
        <a href="#" class="bd-btn bd-btn--success" data-bs-toggle="modal" data-bs-target="#confirmModal"><i class="fa fa-check"></i> {{ __('translate.Confirm Booking') }}</a>
...
<div class="bd-note"><div class="bd-note__label">{{ __('translate.Admin Notes') }}</div><div class="bd-note__text">{{ $booking->admin_notes }}</div></div>
@endif
...
<div class="modal fade" id="confirmModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{ __('translate.Confirm Booking') }}</h5>
        ...
        <form action="{{ route('admin.tourbooking.bookings.confirm', ['id' => $booking->id]) }}" method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="confirmation_message" rows="3"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="bd-btn" data-bs-dismiss="modal">{{ __('translate.Cancel') }}</button>
                <button type="submit" class="bd-btn bd-btn--success">{{ __('translate.Confirm Booking') }}</button>
            </div>
        </form>
    </div></div>
</div>
...
<div class="modal fade" id="paymentStatusModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{ __('translate.Change Payment Status') }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <form action="{{ route('admin.tourbooking.bookings.payment-status', ['booking' => $booking]) }}" method="POST">
            ...
            <div class="modal-footer">
                <button type="button" class="bd-btn" data-bs-dismiss="modal">{{ __('translate.Cancel') }}</button>
                <button type="submit" class="bd-btn bd-btn--success">{{ __('translate.Change Payment Status') }}</button>
            </div>
        </form>
    </div></div>
</div>
